using ePlatform.Api.Core.Exceptions;
using Flurl.Http;
...
namespace ePlatform.Api.Core.Http
{
    public static class FlurlClientExtensions
    {
        ...
        public static IFlurlClient HandleHttp422Async(this IFlurlClient flurlClient)
        {
            return flurlClient.Configure(config =>
            {
                config.OnErrorAsync = async httpCall =>
        ...
        private static async Task Handle422Async(HttpCall call)
        {
            if ((int)call.HttpStatus == 422)
            {
                ...
                if (result.Any())
                {
                    throw new DomainException(result, result.FirstOrDefault().Value.FirstOrDefault(), call.Exception);
                }
            }
        }
